...
<body>
  ...
  if(isset($_POST["addcourseID"])){
    ...
    $query = "INSERT INTO student_course VALUES('$id','$addcourseid')";
    $myDb->doQuery($query);
    unset($_POST["addcourseID"]);
  }
  //if there is a course to be removed
  if(isset($_GET["courseID"])&&isset($_GET["delete"])&&$_GET["delete"]=='true'){
    $deletecourseid=$_GET["courseID"];
    $query = "DELETE FROM student_course WHERE Course='$deletecourseid' AND Student='$id'";
    $_GET["delete"]='false';
    unset($_GET["courseID"]);
    unset($_GET["delete"]);
    $myDb->doQuery($query);
    ...
    header("location: student_courses.php?id=$id&studentname=$studentname&studentsurname=$studentsurname");
  }
  ...
  $query = "SELECT course.ID, course.Name, Credit, teacher.Name as TeacherName, teacher.Surname as TeacherSurname,
            ...
            JOIN student_course ON course.ID=student_course.Course
            WHERE student_course.Student='$id'
						ORDER BY course.ID";
  ...
  <div class="row">
    ...
    <div class="col-md-8">
      <h1 class="text-center"><?php echo $_GET["studentname"].' '.$_GET["studentsurname"] ?></h1>
      <h2 class="text-center">Current semester courses</h2>
      <?php
        ...
        for ($i = 0; $i < $result->num_rows; $i++) {
          ...
          if($year[$i]==date("Y")&&$semester[$i]==$currentsemester){
            ...
            echo '<tr>
                  ...
                  <td class="text-center"><a href="student_courses.php?delete=true&courseID='.$id[$i].'
                  &id='.$_GET['id'].'&studentname='.$_GET['studentname'].'&studentsurname='.$_GET['studentsurname'].'">
        ...
        if($myDb->connected)
        {
          ...
          $courseresult = $myDb->doQuery("SELECT ID, Name from course
                                          ...
                                          SELECT Course FROM student_course WHERE Student='$studentid')");
